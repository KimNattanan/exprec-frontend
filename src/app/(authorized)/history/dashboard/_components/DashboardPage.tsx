...
import { emptyDashboardData, useDashboardContext } from "./DashboardContext"
...
function SearchBox() {
  ...
  const dashboard = useDashboard({
    timeStart: dashboardParams.timeStart,
    timeEnd: dashboardParams.timeEnd,
    queryConfig: {
      enabled: dashboardParams.fetchEnable,
      gcTime: 0,
    },
  });
  const fetchData = () => {
  ...
  useEffect(()=>{
    if(!dashboard.data){
      ...
      setCategoryStatus(new Map<string,boolean>());
      return;
    }
    const categoryStatus = new Map<string,boolean>();
    const arr = dashboard.data.amount_by_category.keys();
